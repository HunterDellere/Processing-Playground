...
void doReset() {
  ...
  maxAngle = random(5,150) * TWO_PI;
  layers = 10;
  float depth = random(1, 3);
  segments = (int)random(500,1000);
  ...
  for (int i = 0; i < depth; i++) {
    ...
    for (float a = 0; a < maxAngle; a += maxAngle/segments) {
      ...
      layer = floor(a/(TWO_PI)); //(floor((segments/layers)));
      //float layer = a%(floor((segments/layers)));
      float r = renderWidth/2 - layer * radius;
