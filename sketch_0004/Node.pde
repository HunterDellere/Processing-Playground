class Node {
  ...
  PVector prevPos;
  float mass, life, m1;
  float resolve = random(pow(2, 5),pow(3,4));
  color hsb;
  ...
  void age() {
    if (life > 0) {
      ...
      float r = dist(renderWidth/2, renderHeight/2, pos.x, pos.y);
      float breathe = sin(life/resolve*TWO_PI) + .1; 
      float potential = 1;
      mass = breathe * m1 * potential;
      life -= 1/resolve;
    }
    else {
      mass = 0;
