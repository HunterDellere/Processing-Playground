...
void doReset() {
  ...
  maxAngle = random(10, 45) * TWO_PI;
  
  float radius = random(6, 11);
  
  layers = (int)(((renderHeight) / radius));
  segments = (int)(layers*random(50, 500));
  float depth = random(1, 7);
  Palette();
  ...
  for (int i = 0; i < depth; i++) {
    ...
    for (float a = random(0, TWO_PI); a < maxAngle; a += maxAngle/layers) {

      float layer = a%(layers*floor((segments/layers)));
      float r = layer * radius;

      float x = midX + r * cos(a);
      float y = midY + r * sin(a);
      float life = 100*abs(cos(i)*sin(i));
      float mass = 100;
