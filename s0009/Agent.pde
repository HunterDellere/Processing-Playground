class Agent {
  ...
  void display(PGraphics r) {
    ...
    PVector noiseV = PVector.fromAngle(noise(pos.x/100, pos.y/100, mass/1000) * maxNoiseAngle).setMag(2);
    float offset = pow(sin(noiseV.heading()), 3)%TAU; //vel.heading()
  ...
  void follow() {
    ...
    PVector centerForce = new PVector(pos.x-midX, pos.y-midY).setMag(pow(dist(pos.x, pos.y, midX, midY), 0.8));
